...
export default function HeaderPhoto() {
  return (
    <div className=" hidden lg:flex flex-row justify-center relative  ">
      ...
      <div className="absolute top-16 flex items-center justify-center w-[400px] h-[400px] bg-[#DFE1FA] dark:bg-[#583FBC] rounded-full">
        <Image
          src={DATA.PROFILE.profilePhotoLink}
          width={1024}
          height={1024}
          alt={`${DATA.PROFILE.profilePhotoLink} profile photo`}
          className="w-96 object-cover rounded-full scale-95"
        />
      </div>
    </div>
  );
